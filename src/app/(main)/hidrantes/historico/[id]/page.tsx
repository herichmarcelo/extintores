...
export default function HistoricoHidrantePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const checklistItems = [
    { id: "localAcessivel", label: "Local Acessível" },
    { id: "sinalizado", label: "Sinalizado" },
    { id: "estadoMangueiras", label: "Estado das Mangueiras" },
    { id: "enroladasCorretamente", label: "Enrolamento Correto" },
    { id: "esguichosNoLocal", label: "Esguichos Presentes" },
    { id: "esguichosBoasCondicoes", label: "Condição dos Esguichos" },
    { id: "semVazamentos", label: "Ausência de Vazamentos" },
    { id: "valvulaFechada", label: "Válvula Fechada" },
    { id: "temChaveStorz", label: "Chave Storz no Local" },
    { id: "estadoPintura", label: "Condição da Pintura" },
    { id: "proximoTesteHidrostatico", label: "Teste Hidrostático Válido" },
  ]

  // Métricas Rápidas
  const totalInspecoes = hidrante.inspecoes?.length || 0
  const lastInspecao = hidrante.inspecoes?.[0]
  ...
  return (
    <div className="min-h-screen bg-[#F4F4F5] pb-28 font-sans">
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="bg-white/20 backdrop-blur-md px-4 py-1.5 rounded-full flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-white uppercase tracking-widest">{hidrante.codigo}</span>
            </div>
          </div>
          ...
          <div className="text-center text-white mb-6">
            ...
            <p className="flex items-center justify-center gap-2 text-white/80 text-sm font-medium">
              <MapPin className="h-4 w-4" /> {hidrante.localizacao}
            </p>
          </div>
        </div>
      </div>

      {/* 2. OVERVIEW METRICS (Flutuando sobre o hero) */}
                  ...
                  <Card className="rounded-3xl border-none shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                    <CardContent className="p-0">
                      ...
                      <div className="p-5 flex flex-col gap-4">
                        <div className="flex justify-between items-start">
                          ...
                          {canEdit() && (
                            <div className="flex gap-1 bg-slate-50 p-1 rounded-xl">
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-400 hover:text-blue-600 rounded-lg" onClick={() => handleEditClick(inspecao)}>
                        ...
                        {/* Barra de Score (Substitui o grid gigante no resumo) */}
                      ...
                      {/* Conteúdo Expandido (Checklist e Fotos) */}
                      <AnimatePresence>
                        {isExpanded && (
                          <motion.div
      ...
      {/* 4. MODAL DE EDIÇÃO OTIMIZADO PARA MOBILE (Abre de baixo pra cima no celular) */}
      {editingInspecao && (
        <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
          <DialogContent className="sm:max-w-xl w-full h-[90vh] sm:h-auto mt-auto sm:mt-0 mb-0 sm:mb-auto rounded-t-[32px] sm:rounded-3xl p-0 flex flex-col overflow-hidden bg-[#F4F4F5]">
              ...
              <form id="edit-form" onSubmit={handleEditSubmit} className="space-y-8">
                ...
                <div className="bg-white p-5 rounded-3xl shadow-sm space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <select name="status" defaultValue={editingInspecao.status} className="w-full h-12 px-4 rounded-xl border border-slate-200 bg-slate-50 text-slate-800 font-bold focus:ring-2 focus:ring-blue-600 outline-none">
            ...
            <div className="p-4 bg-white border-t border-slate-100 shrink-0 flex gap-3">
              ...
              <Button type="submit" form="edit-form" disabled={isSubmitting} className="flex-1 h-14 rounded-2xl font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-600/20">
